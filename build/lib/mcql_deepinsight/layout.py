...
def feature_matrix_for_reducer(X_filled: np.ndarray) -> np.ndarray:
    F = np.asarray(X_filled, dtype=np.float32).T
    F = F - F.mean(axis=1, keepdims=True)
    sd = F.std(axis=1, keepdims=True)
    sd = np.where(sd > 1e-8, sd, 1.0)
    return (F / sd).astype(np.float32)
...
def compute_feature_coordinates(
    ...
    **reducer_params,
    ...
    F = feature_matrix_for_reducer(X_filled)
    ...
    if reducer == "pca":
        ...
        coords = np.stack([np.arange(F.shape[0], dtype=np.float32), np.zeros(F.shape[0], dtype=np.float32)], axis=1)
    elif reducer == "umap":
        try:
            import umap
        except Exception as e:
            raise ImportError("reducer='umap' requires umap-learn.") from e
...
def assign_pixels_rounded_overlap(coords: np.ndarray, image_size: int | tuple[int, int]):
    ...
    rows = np.clip(np.rint(z[:, 1] * (H - 1)).astype(int), 0, H - 1)
    cols = np.clip(np.rint(z[:, 0] * (W - 1)).astype(int), 0, W - 1)
    feature_to_pixel = [(int(r), int(c)) for r, c in zip(rows, cols)]

    flats = rows * W + cols
    counts = np.bincount(flats, minlength=H * W)
